...
class Session extends CHttpSession
{
	...
	public function regenerateID($deleteOldSession = false)
	{
		...
		if($row){ // $row should either be a truey value or a falsey value
			if($deleteOldSession){
				$db->{$this->sessionTableName}->updateOne(array('id' => $oldID), array('$set' => array('id' => $newID)));
			}else{
				unset($row['_id']);
				$row['id'] = $newID;
				$db->{$this->sessionTableName}->insertOne($row);
			}
		}else{
			// shouldn't reach here normally
			$db->{$this->sessionTableName}->insertOne(array(
				'id' => $newID,
	...
	protected function getDbConnection()
	{
		if($this->_db !== null){
			...
			if(($this->_db = Yii::app()->getComponent($id)) instanceof Client){
				return $this->_db->selectDatabase();
			}else{
				throw new Exception(
					Yii::t(
						'yii', 
						'mongoyii\Session.connectionID "{id}" is invalid. Please make sure it refers to the ID of a mongoyii\Client application component.',
						array('{id}' => $id)
					)
				);
			}
		}else{
			return $this->_db = Yii::app()->getComponent('mongodb')->selectDatabase();
		}
	}
	...
	public function writeSession($id, $data)
	{
		...
		try{
			$expire = time() + $this->getTimeout();
			$db = $this->getDbConnection();
			$res = $db->{$this->sessionTableName}->updateOne(array('id' => $id), array('$set' => array(
				'data' => $data,
				'expire' => $expire
			)), array('upsert' => true));
	...
	{
		$this->getDbConnection()->{$this->sessionTableName}->deleteOne(array('id' => $id));
		return true;
	}
	...
	{
		$this->getDbConnection()->{$this->sessionTableName}->deleteMany(array('expire' => array('$lt' => time())));
		return true;
	}
}