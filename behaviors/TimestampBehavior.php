...
namespace pcmnac\mongoyii\behaviors;
...
class TimestampBehavior extends CActiveRecordBehavior
{
	...
	*/
	...
	{
		if($this->timestampExpression instanceof UTCDateTime){
	...
	protected function checkScenarios()
	{
		if(!is_array($this->onScenario) or !is_array($this->notOnScenario)){
			throw new Exception('onScenario and notOnScenario must be an array');
		}
		if(count($this->onScenario)){
			if(count($this->notOnScenario)){
				throw new Exception('You can not specify both the parameter and onScenario notOnScenario');
			}
